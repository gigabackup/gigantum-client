...
chown giguser /opt/redirect.py

# RUN mitmproxy to log traffic.
# E.g., for RStudio, LBENDPOINT will be set to labbook_ip:8787
gosu giguser:root nohup mitmdump -s /opt/redirect.py --mode reverse:$LBENDPOINT --setheader :~q:Host:localhost:8080 -w $LOGFILE_NAME < /dev/null >/dev/null 2>&1 &
