...
@pytest.mark.deletePackageTest
class TestDeletePackage:
    """Includes test methods for basic package creation,deletion and its dependent tests"""
    ...
    def test_delete_package(self, clean_up_project):
        ...
        is_checked = package_list.check_package(package_gtmunit1)
        ...
        is_verified = package_list.check_package(package_gtmunit2)
        assert is_verified, "Could not verify package and it's version"

        # Verify package and it's version
        package_gtmunit3 = package("gtmunit3", "5.0")
        is_verified = package_list.check_package(package_gtmunit3)
        assert is_verified, "Could not verify package and it's version"

        # Remove all packages
        is_removed = package_list.delete_all_packages()
        assert is_removed, "could not remove packages"
    ...
    def __add_packages(self, package_list) -> None:
        ...
        Args:
            package_list: Instance of Project Package Page
        ...
        assert is_clicked, "Could not click Add button (gtmunit1)"

        # Input package name
        is_package_title_typed = package_list.type_package_name("gtmunit2")
        assert is_package_title_typed, "Could not type package gtmunit2"

        # Click on Add button
        is_clicked = package_list.click_add()
        assert is_clicked, "Could not click Add button (gtmunit2)"

        # Input package name
        is_package_title_typed = package_list.type_package_name("gtmunit3")
        assert is_package_title_typed, "Could not type package gtmunit3"

        # Click on Add button
        is_clicked = package_list.click_add()
        assert is_clicked, "Could not click Add button (gtmunit3)"

        # Monitor package list status
        is_status_changed = package_list.monitor_package_list_status("3 added", 30)
